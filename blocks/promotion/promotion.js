...
/* Adobe Target config is read from page metadata so each project can set
 * its own values without editing this block:
 *   - metadata `at-property` → the Target at_property token
 *   - metadata `atjs-url`     → URL/path to the at-lsig.js bootstrap
 * `atjs-url` falls back to the at-lsig.js already shipped in scripts/ and
 * loaded lazily by scripts/delayed.js; `at-property` has no default here
 * since it is specific to the Adobe Target account behind this site. */
...
/* ────────────────────────────────────────────
 * at-lsig.js – wait for it to load (it comes via
 * delayed.js shortly after page load, or is loaded
 * eagerly below), then use getOffers() so Target
 * gets full page context, ECID visitor identity
 * and browsing history needed for page-URL-based
 * audiences.
 * ──────────────────────────────────────────── */

/**
 * Resolve Adobe Target config from page metadata, with fallbacks.
 * Returns the at_property token (if configured) and the resolved
 * at-lsig.js URL.
 */
function getTargetConfig() {
  const codeBase = window.hlx?.codeBasePath || '';
  const atProperty = getMetadata('at-property');
  const atjsMeta = getMetadata('atjs-url');
  let atjsUrl = `${codeBase}${DEFAULT_ATJS_PATH}`;
  if (atjsMeta) {
    // Absolute URL is used as-is; a relative path is resolved against the code base.
    atjsUrl = /^https?:\/\//i.test(atjsMeta)
      ? atjsMeta
      : `${codeBase}${atjsMeta.startsWith('/') ? '' : '/'}${atjsMeta}`;
  }

  log('Target config:', { atProperty: atProperty || '(none)', atjsUrl });
  return { atProperty, atjsUrl };
}

/**
 * Eagerly load at-lsig.js if not already loaded (instead of waiting
 * for scripts/delayed.js to load it later). Sets the required globals
 * first.
 */
async function ensureAtJsLoading() {
  ...
  const { atProperty, atjsUrl } = getTargetConfig();
  if (document.querySelector(`script[src="${atjsUrl}"]`)) return;

  log('Eagerly loading at-lsig.js from promotion block:', atjsUrl);

  if (!window.targetGlobalSettings) {
    window.targetGlobalSettings = { bodyHidingEnabled: false };
  }
  if (!window.targetPageParams && atProperty) {
    window.targetPageParams = () => ({ at_property: atProperty });
  }

  await loadScript(atjsUrl);
}

async function waitForAtJs() {
  log('Waiting for at-lsig.js to initialise...');
  await ensureAtJsLoading();

  return new Promise((resolve) => {
    if (window.adobe?.target?.getOffers) {
      log('at-lsig.js already available');
      resolve(true);
      return;
    }
    ...
    const timer = setInterval(() => {
      if (window.adobe?.target?.getOffers) {
        clearInterval(timer);
        log(`at-lsig.js ready after ${Date.now() - start}ms`);
        resolve(true);
      } else if (Date.now() - start > ATJS_MAX_WAIT_MS) {
        clearInterval(timer);
        logWarn(`at-lsig.js did not load within ${ATJS_MAX_WAIT_MS}ms timeout`);
        resolve(false);
      }
...
/**
 * Extract the first JSON offer content from an at-lsig.js getOffers() response.
...
function extractJsonOffer(response) {
  ...
  const mboxWithOffer = mboxes.find((mbox) => (mbox.options || []).some((o) => o.type === 'json'));
  if (mboxWithOffer) {
    const jsonOffer = mboxWithOffer.options.find((o) => o.type === 'json');
    log(`  ✓ Found JSON offer in named mbox "${mboxWithOffer.name}"`);
...
async function fetchTargetOffer(mbox) {
  ...
  if (!atjsReady) {
    logWarn('at-lsig.js did not load – skipping Target personalisation');
    return null;
  }
...
async function renderCard(block, cfItem, isAuthor, source = 'default', displayStyle = '', alignment = '') {
  ...
  // AEM GraphQL response fields use a fixed `_path`/`_publishUrl`/`_authorUrl` naming
  // convention that isn't ours to rename.
 ...
 *  2. In the background, wait for at-lsig.js → call getOffers()
 *  3. If Target returns a personalized offer, swap the card
 *
 * This avoids a blank gap while at-lsig.js loads
...
export default async function decorate(block) {
  ...
  log('Config:', {
    contentPath, variation, displayStyle, alignment, mboxName,
  });
